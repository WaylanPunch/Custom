...
public class DemoAppWidget extends AppWidgetProvider {
    ...
    // 更新小插件的广播
    public static final String CHANGE_STATE = "com.way.android5widget.action.CHANGE_STATE";
    ...
    private PendingIntent getUpdateIntent(Context context, boolean isStart) {
        // 获取当前的组件
        ComponentName widget = new ComponentName(context, DemoAppWidget.class);

        // 获取布局, 并设置关闭显示
        RemoteViews rv = new RemoteViews(context.getPackageName(), R.layout.app_widget_demo);
        if (isStart) {
        ...
        Intent alertIntent = new Intent(context, DemoAppWidget.class);
